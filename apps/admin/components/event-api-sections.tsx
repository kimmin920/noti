...
interface EventRuleBuilderCardProps {
  eventRuleForm: any;
...
interface EventTestRunnerCardProps {
...
export function EventRuleBuilderCard({
  eventRuleForm,
  setEventRuleForm,
  ...
  return (
    <Card className="glass">
      <CardHeader>
        <CardTitle>이벤트 만들기</CardTitle>
        <CardDescription>
          이벤트 키, 채널 전략, 연결 템플릿을 정의합니다. 저장 후에는 다른 페이지에서 바로 테스트할 수 있습니다.
        </CardDescription>
      </CardHeader>
...
export function EventTestRunnerCard({
  ...
  executeEventTest,
  eventRules
}: EventTestRunnerCardProps) {
  return (
    <Card className="glass">
      <CardHeader>
        <CardTitle>이벤트 테스트</CardTitle>
        <CardDescription>
          저장된 이벤트를 선택한 뒤 API 요청 형식 그대로 실행합니다. 실제 외부 연동팀이 보는 예시와 같은 구조로 보여줍니다.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950/60">
          <div className="flex flex-col gap-2">
            ...
            <div className="text-lg font-semibold">등록된 이벤트를 선택하고 바로 실행합니다</div>
            <div className="text-sm text-muted-foreground">
              수신자 정보와 변수만 넣으면 실제 API 요청 예시와 payload를 같은 화면에서 확인할 수 있습니다.
            </div>
          </div>
