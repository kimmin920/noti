...
export function SidebarSetupBox({
  ...
  const hasAttentionSms = resources.sms === "rejected" || resources.sms === "supplement";
    ...
    <div className={`setup-box${allDone ? " all-done" : ""}`}>
      <div className="setup-box-top">
        <div className="setup-box-title">
          ...
          {allDone ? "모든 발신 자원 준비 완료" : hasAttentionSms ? "확인이 필요합니다" : "초기 설정이 필요합니다"}
        </div>
      ...
      <div className="setup-row">
        <span className={`dot ${resources.sms === "active" ? "done" : resources.sms === "rejected" || resources.sms === "supplement" ? "rejected" : "pending"}`} />
        <span>{smsText}</span>
      </div>
      <div className="setup-row">
